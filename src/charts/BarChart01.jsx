...
function BarChart01({
  ...
  useEffect(() => {
    //The if block allows for the chart to be updated after an api call
    //Starts here and ends ... }, []); return (
      ...
      plugins: [{
        ...
        afterUpdate(c, args, options) {
          ...
          items.forEach((item) => {
            ...
            var valueText = document.createTextNode(theValue);
            if(currency == true){
              valueText = document.createTextNode(formatValueCurrency(theValue));
            }
            var labelText = document.createTextNode(item.text);
            if(labelText.data == 'undefined'){
              labelText.data = "";
            };
